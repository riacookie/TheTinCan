module.exports = () => new Promise((resolve, reject) => {
    ...
    Client.on('message', async message => {
        if (!message.author.bot && message.content.startsWith(process.env.prefix)) {
            ...
            if (bot.commands.cmds[cmd] && (bot.commands.cmds[cmd] == 'identity' || (!(await management.isBlacklisted(mention.user.id))))) {
                try {
            ...
            else if (!(await management.isBlacklisted(mention.user.id)) && wandbox.languages.lower.includes(cmd.toLowerCase()) || wandbox.languages.lower.includes(cmd.toLowerCase() + ' ' + firstWord(shiftWord(message.content.toLowerCase())))) {
                try {
